...
public sealed partial class ProveedorSimulado : IProveedorLenguaje
{
    ...
    public Task<RespuestaLenguaje> CompletarAsync(PeticionLenguaje peticion, CancellationToken cancelacion = default)
    {
        ...
        // El modo se deduce de la conversación recibida: cuando las
        // instrucciones traen un catálogo de herramientas, se está actuando
        // como agente; cuando se pide JSON sin catálogo, se está clasificando;
        // en cualquier otro caso, se está respondiendo una consulta documental.
        var contenido = EsModoAgente(peticion.Mensajes)
            ? ResponderComoAgente(peticion.Mensajes)
            : peticion.RespuestaJson
