...
export const api = {
    ...
    consultarAgente: (pregunta, autorizarEscritura = false) =>
        pedir('/agente/consultas', {
            method: 'POST',
            body: JSON.stringify({ pregunta, autorizarEscritura }),
        }),

    listarHerramientas: () => pedir('/agente/herramientas'),

    generarReporte: () => pedir('/agente/reportes/ejecutivo', { method: 'POST' }),

    saludOrquestacion: () => pedir('/orquestacion/salud'),

    listarEventos: (filtros = {}) => pedir(`/orquestacion/eventos${consulta(filtros)}`),
};
